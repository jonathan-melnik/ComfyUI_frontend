...
function find_all_upstream(node_id, links_added) {
  ...
  node?.inputs?.forEach((input) => {
    ...
    if (link_id) {
      ...
      if (link)
        all_upstream.push({ id: link.origin_id, slot: link.origin_slot })
    }
  })
  links_added.forEach((la) => {
    ...
    if (get_real_node(la.downstream).id == node.id) {
      all_upstream.push({
        id: la.upstream,
        slot: la.upstream_slot,
        ue: la.controller.toString()
      })
    }
  })
  if (node.id != get_group_node(node.id).id) {
    ...
    const indx = group_data.nodeData.nodes.findIndex(
      (n) => n.pos[0] == node.pos[0] && n.pos[1] == node.pos[1]
    )
    if (indx >= 0) {
      if (
        GroupNodeHandler.getGroupData(app.graph._nodes_by_id[grp_nd])
          ?.linksTo?.[indx]
      ) {
        // links within group
        Object.values(
          GroupNodeHandler.getGroupData(app.graph._nodes_by_id[grp_nd]).linksTo[
            indx
          ]
        ).forEach((internal_link) => {
          all_upstream.push({
            id: `${grp_nd}:${internal_link[0]}`,
            slot: internal_link[1]
          })
        })
      }
      if (
        GroupNodeHandler.getGroupData(app.graph._nodes_by_id[grp_nd])
          .oldToNewInputMap?.[indx]
      ) {
        // links out of group
        Object.values(
          GroupNodeHandler.getGroupData(app.graph._nodes_by_id[grp_nd])
...
function recursive_follow(
  ...
  count += 1
  if (stack.includes(node.id.toString()))
    throw new LoopError(node.id, new Set(stack), new Set(ues))
...
function node_in_loop(live_nodes, links_added) {
  ...
  while (nodes_to_check.length > 0) {
    ...
    nodes_to_check = nodes_to_check.filter(
      (nid) => !nodes_cleared.has(nid.toString())
    )
  }
...
function node_is_live(node) {
  ...
  Logger.log(
    Logger.ERROR,
    `node ${node.id} has mode ${node.mode} - I only understand modes 0, 2 and 4`
  )
  return true
}
...
function is_in_group(node_id, group_node) {
  return group_node
    .getInnerNodes()
    .find((inner_node) => inner_node.id == node_id)
}
...
function get_group_node(node_id, level = Logger.ERROR) {
  ...
  if (!gn)
    gn = all_group_nodes().find((group_node) => is_in_group(nid, group_node))
...
function get_real_node(node_id, level = Logger.INFORMATION) {
  ...
  if (!rn && nid.includes(':'))
    rn =
      app.graph._nodes_by_id[nid.split(':')[0]]?.getInnerNodes()[
        nid.split(':')[1]
      ]
  if (!rn) {
    all_group_nodes().forEach((node) => {
      if (!rn)
        rn = node.getInnerNodes().find((inner_node) => inner_node.id == nid)
    })
  }
  if (!rn)
    Logger.log(
      level,
      `get_real_node couldn't find ${node_id} - ok during loading, shortly after node deletion etc.`
    )
  return rn
}
...
function is_UEnode(node_or_nodeType) {
  ...
  return (
    title &&
    (title.startsWith('Anything Everywhere') ||
...
export function defineProperty(instance, property, desc) {
  ...
  if (existingDesc?.configurable === false) {
    throw new Error(
      `Error: Cannot define un-configurable property "${property}"`
    )
  }
