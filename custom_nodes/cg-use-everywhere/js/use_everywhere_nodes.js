...
function get_widget_or_input_values(node_obj, widget_id) {
  if (node_obj.widgets[widget_id]?.type.startsWith(CONVERTED_TYPE)) {
    try {
      ...
      const upstream_node_obj = get_real_node(
        connection.link.origin_id.toString()
      )
      const widget = upstream_node_obj.widgets.find(
        (w) =>
          w.name.toLowerCase() ==
          upstream_node_obj.outputs[
            connection.link.origin_slot
          ].name.toLowerCase()
      )
      return widget.value
    } catch (error) {
...
function get_available_input_name(inputs, the_input, type) {
  ...
  inputs.forEach((input) => {
    if (input != the_input) used_names.push(input.name)
  })
...
function get_connection(node, i, override_type) {
  ...
  if (in_link) {
    ...
    if (
      node.in_group_with_data.linksTo[node.index] &&
      node.in_group_with_data.linksTo[node.index][i]
    ) {
      const group_style_link = node.in_group_with_data.linksTo[node.index][i]
      link = {
        origin_id: node.getInnerNodesOfGroup()[group_style_link[0]].id,
        origin_slot: group_style_link[1]
      }
      if (!override_type) type = group_style_link[5]
    } else {
      // group external input
      const group_node = get_group_node(node.id)
      const group_node_input =
        group_node.inputs[
          node.in_group_with_data.oldToNewInputMap[node.index][i]
        ]
      ...
      if (link_n) {
        ...
        if (!override_type)
          type =
            app.graph._nodes_by_id[link.origin_id].outputs[link.origin_slot]
              .type
        // update the group input node... and the link type
        group_node_input.type = type
        group_node_input.name = get_available_input_name(
          group_node.inputs,
          group_node_input,
          type
        )
...
function add_ue_from_node(ues, node) {
  ...
  if (node.type === 'Anything Everywhere?') {
    ...
    if (connection.link) {
      ...
      ues.add_ue(
        ...
        0,
        connection.type,
        [connection.link.origin_id.toString(), connection.link.origin_slot],
        r0,
        r1,
        r2,
        10
    ...
    for (var i = 0; i < 2; i++) {
      ...
      if (connection.link)
        ues.add_ue(
          ...
          new RegExp(
            [
              '(_|\\b)pos(itive|_|\\b)|^prompt|正面',
              '(_|\\b)neg(ative|_|\\b)|负面'
            ][i]
          ),
          undefined,
          5
  ...
  if (node.type === 'Anything Everywhere3') {
    for (var i = 0; i < 3; i++) {
      const connection = get_connection(node, i)
      if (connection.link)
        ues.add_ue(node, i, connection.type, [
          connection.link.origin_id.toString(),
          connection.link.origin_slot
        ])
    }
