...
export class UpdateBlocker {
  static depth = 0
  static push() {
    UpdateBlocker.depth += 1
  }
  static pop() {
    UpdateBlocker.depth -= 1
  }
  static blocking() {
    return UpdateBlocker.depth > 0
  }
}
...
function update_input_label(node, slot, app) {
  if (node.input_type[slot]) {
    ...
    node.inputs[slot].color_on =
      app.canvas.default_connection_color_byType[node.input_type[slot]]
  } else {
...
class LinkRenderController {
  ...
  static instance(tga) {
    ...
    if (tga && !this._instance.the_graph_analyser)
      this._instance.the_graph_analyser = tga
  ...
  request_link_list_update() {
    ...
    this.the_graph_analyser
      .analyse_graph()
      .then(this.reload_resolve, this.reload_reject) // an async call is a promise; pass it two callbacks
  ...
  _highlight_ue_connections(node, ctx) {
    ...
    if (this.ue_list.all_connected_inputs) {
      this.ue_list.all_connected_inputs(node).forEach((ue_connection) => {
        if (!ue_connection.control_node) {
          // control node deleted...
          this.mark_link_list_outdated()
          return
        }
        var pos2 = node.getConnectionPos(
          true,
          ue_connection.input_index,
          this.slot_pos1
        )
        pos2[0] -= node.pos[0]
        pos2[1] -= node.pos[1]
  ...
  node_in_ueconnection(ue_connection, id) {
    if (
      ue_connection.control_node &&
      get_group_node(ue_connection.control_node.id)?.id == id
    )
      return true
    if (
      ue_connection.sending_to &&
      get_group_node(ue_connection.sending_to.id)?.id == id
    )
      return true
  }

  any_node_in_ueconnection(ue_connection, list_of_nodes) {
    return Object.values(list_of_nodes).find((node) =>
      this.node_in_ueconnection(ue_connection, node.id)
    )
      ? true
      : false
  }
  ...
  _render_all_ue_links(ctx) {
    ...
    if (
      app.ui.settings.getSettingValue('AE.stop.animation.running') &&
      this.queue_size > 0
    )
      animate = 0
    ...
    this.ue_list.all_ue_connections().forEach((ue_connection) => {
      ...
      if (
        ...
        this.node_in_ueconnection(ue_connection, app.canvas.node_over.id)
      )
        show = true
      if (
        (mode == 1 || mode == 3) &&
        this.any_node_in_ueconnection(ue_connection, app.canvas.selected_nodes)
      )
        show = true
    ...
    if (animate > 0) {
      /*
            ...
            */
      const timeout = any_links_shown
        ? animate % 2 == 1
          ? 30
          : 100
        : (mode == 2 || mode == 3) && any_links
          ? 200
          : -1
  ...
  _render_ue_link(ue_connection, ctx, animate) {
    try {
      ...
      const source_index = input_source
        ? ue_connection.control_node_input_index
        : -1 - ue_connection.control_node_input_index
      const pos1 = get_group_node(
        ue_connection.control_node.id
      ).getConnectionPos(input_source, source_index, this.slot_pos2)
      ...
      const sta_direction =
        Math.abs(delta_y) > Math.abs(delta_x)
          ? delta_y > 0
            ? LiteGraph.DOWN
            : LiteGraph.UP
          : input_source && delta_x < 0
            ? LiteGraph.LEFT
            : LiteGraph.RIGHT
      ...
      Logger.log(
        Logger.PROBLEM,
        `Couldn't render UE link ${ue_connection}. That's ok if something just got deleted.`
      )
    }
  }
...
export {
  ...
  update_input_label,
  nodes_in_my_group,
  nodes_not_in_my_group,
  nodes_in_groups_matching,
  nodes_my_color,
  nodes_not_my_color,
  indicate_restriction
}
