...
class FastGroupsService {
  ...
  removeFastGroupNode(node) {
    ...
    if (
      !((_a = this.fastGroupNodes) === null || _a === void 0
        ? void 0
        : _a.length)
    ) {
  ...
  recomputeInsideNodesForGroup(group) {
    ...
    for (const node of nodes) {
      ...
      if (
        !node_bounding ||
        !LiteGraph.overlapBounding(group._bounding, node_bounding)
      ) {
        continue
      }
  ...
  getGroupsUnsorted(now) {
    ...
    if (
      ...
      (!this.groupsUnsorted.length ||
        now - this.msLastUnsorted > this.msThreshold)
    ) {
      this.groupsUnsorted = [...graph._groups]
      for (const group of this.groupsUnsorted) {
        this.recomputeInsideNodesForGroup(group)
        group._rgthreeHasAnyActiveNode = group._nodes.some(
  ...
  getGroupsAlpha(now) {
    const graph = app.graph
    if (
      !this.groupsSortedAlpha.length ||
      now - this.msLastAlpha > this.msThreshold
    ) {
  ...
  getGroupsPosition(now) {
    const graph = app.graph
    if (
      !this.groupsSortedPosition.length ||
      now - this.msLastPosition > this.msThreshold
    ) {
      this.groupsSortedPosition = [...this.getGroupsUnsorted(now)].sort(
        (a, b) => {
