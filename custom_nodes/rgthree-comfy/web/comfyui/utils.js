...
export function addMenuItem(node, _app, config, after = 'Shape') {
  ...
  node.prototype.getExtraMenuOptions = function (canvas, menuOptions) {
    oldGetExtraMenuOptions &&
      oldGetExtraMenuOptions.apply(this, [canvas, menuOptions])
...
export function addMenuItemOnExtraMenuOptions(
  ...
  let idx = menuOptions
    ...
    .reverse()
    .findIndex((option) =>
      option === null || option === void 0 ? void 0 : option.isRgthree
    )
  if (idx == -1) {
    idx =
  ...
  menuOptions.splice(idx, 0, {
    ...
    has_submenu: !!(subMenuOptions === null || subMenuOptions === void 0
      ? void 0
      : subMenuOptions.length),
    isRgthree: true,
    callback: (value, _options, event, parentMenu, _node) => {
      if (
        !!(subMenuOptions === null || subMenuOptions === void 0
          ? void 0
          : subMenuOptions.length)
      ) {
      ...
      config.callback &&
        config.callback(
          node,
          value === null || value === void 0 ? void 0 : value.content
        )
    }
  })
}
export function addConnectionLayoutSupport(
  ...
  addMenuItem(node, app, {
    ...
    prepareValue: (value, node) => {
      ...
      if (
        !values[1] &&
        !((_a = node.outputs) === null || _a === void 0 ? void 0 : _a.length)
      ) {
        values[1] = LAYOUT_LABEL_OPPOSITES[values[0]]
      }
      if (
        !LAYOUT_LABEL_TO_DATA[values[0]] ||
        !LAYOUT_LABEL_TO_DATA[values[1]]
      ) {
...
export function setConnectionsLayout(node, newLayout) {
  ...
  if (
    !LAYOUT_LABEL_TO_DATA[newLayout[0]] ||
    !LAYOUT_LABEL_TO_DATA[newLayout[1]]
  ) {
...
export function setConnectionsCollapse(node, collapseConnections = null) {
  ...
  collapseConnections =
    collapseConnections !== null
      ? collapseConnections
      : !node.properties['collapse_connections']
...
export function getConnectionPosForLayout(node, isInput, slotNumber, out) {
  ...
  const offset =
    (_a = node.constructor.layout_slot_offset) !== null && _a !== void 0
      ? _a
  ...
  cxn.dir = data[0]
  if (
    (node.size[0] == 10 || node.size[1] == 10) &&
    node.properties['connections_dir']
  ) {
  ...
  if (side === 'Left') {
    if (node.flags.collapsed) {
      ...
      toggleConnectionLabel(
        cxn,
        !isInput || collapseConnections || !!node.hideSlotLabels
      )
      out[0] = node.pos[0] + offset
      if (
        (_b = node.constructor) === null || _b === void 0
          ? void 0
          : _b.type.includes('Reroute')
      ) {
    ...
    if (node.flags.collapsed) {
      ...
      if (
        (_c = node.constructor) === null || _c === void 0
          ? void 0
          : _c.type.includes('Reroute')
      ) {
        out[1] = node.pos[1] + node.size[1] * 0.5
      } else {
        out[1] =
...
;(function (PassThroughFollowing) {
  ...
  PassThroughFollowing[(PassThroughFollowing['REROUTE_ONLY'] = 2)] =
    'REROUTE_ONLY'
...
export function shouldPassThrough(
  ...
  var _a
  const type =
    (_a = node === null || node === void 0 ? void 0 : node.constructor) ===
      null || _a === void 0
      ? void 0
      : _a.type
...
export function getConnectedNodesInfo(
  ...
  if (
    ...
    shouldPassThrough(currentNode, passThroughFollowing)
  ) {
    let linkIds
    ...
    if (dir == IoDirection.OUTPUT) {
      if (slot != null) {
        linkIds = [
          ...(((_b =
            (_a = currentNode.outputs) === null || _a === void 0
              ? void 0
              : _a[slot]) === null || _b === void 0
            ? void 0
            : _b.links) || [])
        ]
      } else {
        linkIds =
          ((_c = currentNode.outputs) === null || _c === void 0
            ? void 0
            : _c.flatMap((i) => i.links)) || []
      }
    } else {
      if (slot != null) {
        linkIds = [
          (_e =
            (_d = currentNode.inputs) === null || _d === void 0
              ? void 0
              : _d[slot]) === null || _e === void 0
            ? void 0
            : _e.link
        ]
      } else {
        linkIds =
          ((_f = currentNode.inputs) === null || _f === void 0
            ? void 0
            : _f.map((i) => i.link)) || []
      }
    }
    ...
    for (const linkId of linkIds) {
      ...
      const travelFromSlot =
        dir == IoDirection.OUTPUT ? link.origin_slot : link.target_slot
      const connectedId =
        dir == IoDirection.OUTPUT ? link.target_id : link.origin_id
      const travelToSlot =
        dir == IoDirection.OUTPUT ? link.target_slot : link.origin_slot
      originTravelFromSlot =
        originTravelFromSlot != null ? originTravelFromSlot : travelFromSlot
      ...
      if (rootNodes.some((n) => n.node == originNode)) {
        console.log(
          `${startNode.title} (${startNode.id}) seems to have two links to ${originNode.title} (${originNode.id}). One may be stale: ${linkIds.join(', ')}`
        )
      } else {
        rootNodes.push({
          node: originNode,
...
function getTypeFromSlot(slot, dir, skipSelf = false) {
  ...
  if (!skipSelf && type != null && type != '*') {
    return {
      type: type,
      label: slot === null || slot === void 0 ? void 0 : slot.label,
      name: slot === null || slot === void 0 ? void 0 : slot.name
    }
  }
  ...
  for (const link of links) {
    ...
    if (
      (connectedSlot === null || connectedSlot === void 0
        ? void 0
        : connectedSlot.type) != null &&
      (connectedSlot === null || connectedSlot === void 0
        ? void 0
        : connectedSlot.type) != '*'
    ) {
...
export async function replaceNode(existingNode, typeOrNewNode, inputNameMap) {
  ...
  const newNode =
    typeof typeOrNewNode === 'string'
      ? LiteGraph.createNode(typeOrNewNode)
      : typeOrNewNode
  ...
  for (const [index, input] of existingNode.inputs.entries()) {
    ...
    if (linkId) {
      ...
      links.push({
        ...
        targetNode: newNode,
        targetSlot: (
          inputNameMap === null || inputNameMap === void 0
            ? void 0
            : inputNameMap.has(input.name)
        )
...
export function getSlotLinks(inputOrOutput) {
  ...
  if (
    (_a = inputOrOutput.links) === null || _a === void 0 ? void 0 : _a.length
  ) {
...
export async function matchLocalSlotsToServer(node, direction, serverNodeData) {
  ...
  const serverSlotNames =
    direction == IoDirection.INPUT
      ? Object.keys(
          ((_a = serverNodeData.input) === null || _a === void 0
            ? void 0
            : _a.optional) || {}
        )
  ...
  const serverSlotTypes =
    direction == IoDirection.INPUT
      ? Object.values(
          ((_b = serverNodeData.input) === null || _b === void 0
            ? void 0
            : _b.optional) || {}
        ).map((i) => i[0])
      : serverNodeData.output
  const slots = direction == IoDirection.INPUT ? node.inputs : node.outputs
  let firstIndex = slots.findIndex(
    (o, i) => i !== serverSlotNames.indexOf(o.name)
  )
  if (firstIndex > -1) {
    const links = {}
    slots.map((slot) => {
      ...
      ;(_a = links[slot.name]) === null || _a === void 0
        ? void 0
        : _a.push(...getSlotLinks(slot))
    })
      ...
      if (currentNodeSlot > -1) {
        for (const linkData of slotLinks) {
          if (direction == IoDirection.INPUT) {
            ...
            if (
              nextNode &&
              ((_c = nextNode.constructor) === null || _c === void 0
                ? void 0
                : _c.type.includes('Reroute'))
            ) {
...
export function isValidConnection(ioA, ioB) {
  ...
  if (!isValid) {
    let areCombos =
      (typeA.includes(',') && typeB === 'COMBO') ||
      (typeA === 'COMBO' && typeB.includes(','))
    if (areCombos) {
      const nameA = ioA.name
        .toUpperCase()
        .replace('_NAME', '')
        .replace('CKPT', 'MODEL')
      const nameB = ioB.name
        .toUpperCase()
        .replace('_NAME', '')
        .replace('CKPT', 'MODEL')
    ...
    (nodes === null || nodes === void 0
      ...
      : nodes.filter((n) => {
          ...
          return (
            ...
            ((_a = n.constructor.nodeData) === null || _a === void 0
              ? void 0
              : _a.output_node)
          )
        })) || []
