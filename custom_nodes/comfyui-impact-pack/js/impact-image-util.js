...
async function loadImageFromUrl(image, node_id, v, need_to_load) {
  ...
  if (item) {
    ...
    let res = await api.fetchApi('/impact/set/pb_id_image' + params, {
      cache: 'no-store'
    })
    if (res.status == 200) {
      ...
      if (need_to_load) {
        image.src = api.apiURL(
          `/view?filename=${item.filename}&type=${item.type}&subfolder=${item.subfolder}`
        )
      }
      return pb_id
    } else {
...
async function loadImageFromId(image, v) {
  let res = await api.fetchApi('/impact/get/pb_id_image?id=' + v, {
    cache: 'no-store'
  })
  if (res.status == 200) {
...
app.registerExtension({
  ...
  nodeCreated(node, app) {
    if (
      node.comfyClass == 'PreviewBridge' ||
      node.comfyClass == 'PreviewBridgeLatent'
    ) {
    ...
    if (node.comfyClass == 'ImageReceiver') {
      ...
      let stw_widget = node.widgets.find(
        (obj) => obj.name === 'save_to_workflow'
      )
      w._value = ''

      Object.defineProperty(w, 'value', {
        set(v) {
          if (v != '[IMAGE DATA]') w._value = v
        },
        get() {
          const stackTrace = new Error().stack
          if (
            !stackTrace.includes('draw') &&
            !stackTrace.includes('graphToPrompt') &&
            stackTrace.includes('app.js')
          ) {
            return '[IMAGE DATA]'
          } else {
            if (stw_widget.value) return w._value
            else return ''
          }
        }
      })
        ...
        get() {
          if (this._img == undefined && w.value != '') {
            ...
            try {
              ...
              let res = api
                .fetchApi('/view/validate' + params, { cache: 'no-store' })
