...
import { customAlert, customConfirm, show_message, sleep } from './common.js'
...
export function getPureName(node) {
  ...
  if (category) {
    ...
    node.comfyClass.startsWith('workflow/') ||
    node.comfyClass.startsWith(`workflow${SEPARATOR}`)
  ) {
...
async function save_as_component(
  node,
  version,
  author,
  prefix,
  ...
  } else show_message(`Failed to save component.`)
}
...
function versionCompare(v1, v2) {
  ...
  if (v1 && v1 != '') {
    ...
    if (ver1.length == 2) ver1.push(0)
    else ver1[2] = parseInt(ver2[2])
  } else {
    ver1 = [0, 0, 0]
  }
  ...
  if (v2 && v2 != '') {
    ...
    if (ver2.length == 2) ver2.push(0)
    else ver2[2] = parseInt(ver2[2])
  } else {
    ver2 = [0, 0, 0]
  }
  ...
  for (const item of items) {
    if (item.kind == 'string' && item.type == 'text/plain') {
      ...
      try {
        ...
        if (
          json_data.kind == 'ComfyUI Components' &&
...
export class ComponentBuilderDialog extends ComfyDialog {
  ...
  invalidateControl() {
    ...
    this.save_button = $el('button', {
      ...
      onclick: () => {
        save_as_component(
          self.target_node,
          self.version_string.value.trim(),
          self.author.value.trim(),
          self.node_prefix.value.trim(),
          self.getNodeName(),
          self.getPackName(),
          self.category.value.trim()
        )
      }
    })
    ...
    let version_string = this.createLabeledInput(
    ...
    let author = this.createLabeledInput(
      'input author (e.g. Dr.Lt.Data)',
      'Author : ',
      default_author
    )
    this.author = author[1]

    let node_prefix = this.createLabeledInput(
      'input node prefix (e.g. mypack)',
      '*Prefix : ',
      default_prefix
    )
    this.node_prefix = node_prefix[1]

    let manual_nodename = this.createLabeledInput(
      'input node name (e.g. MAKE_BASIC_PIPE)',
      'Nodename : ',
      default_nodename
    )
    this.manual_nodename = manual_nodename[1]

    let manual_packname = this.createLabeledInput(
      'input pack name (e.g. mypack)',
      'Packname : ',
      default_packname
    )
    this.manual_packname = manual_packname[1]

    let category = this.createLabeledInput(
      'input category (e.g. util/pipe)',
      'Category : ',
      default_category
    )
    ...
    const content = $el('div.comfy-modal-content', [
      $el('tr.cm-title', {}, [
        $el('font', { size: 6, color: 'white' }, [
          `ComfyUI-Manager: Component Builder`
        ])
      ]),
      $el('br', {}, []),
  ...
  createAuthorModeCheck() {
    ...
    const check_label = $el('label', { for: 'author-mode' }, [
      'Enable author mode'
    ])
    ...
    check.onchange = () => {
      ...
      if (!check.checked) {
        ...
        customAlert(
          'If you are not the author, it is not recommended to change the version, as it may cause component update issues.'
        )
      }
    }
  ...
  createNodeLabel() {
    ...
    else label.textContent = ' _::' + getPureName(this.target_node)
    return label
  }

  createLabeledInput(placeholder, label, value) {
    let textbox = $el(
      'input.cb-widget-input',
    ...
    textbox.onchange = () => {
      ...
      this.node_label.textContent =
        this.node_prefix.value + '::' + this.manual_nodename.value
    }
    let row = $el('span.cb-widget', {}, [
      $el('span.cb-widget-input-label', label),
      textbox
    ])
  ...
  createPackListCombo() {
    ...
    let default_packname_option = {
      value: '##manual',
      text: 'Packname: Manual'
    }

    combo.appendChild($el('option', default_packname_option, []))
    for (let name in pack_map) {
      combo.appendChild(
        $el('option', { value: name, text: 'Packname: ' + name }, [])
      )
    }
...
try {
  api
    ...
    .then((data) => {
      current_component_policy = data
    })
} catch {}

function getChangedVersion(groupNodes) {
  if (!Object.keys(pack_map).length || !groupNodes) return null
...
app.loadGraphData = async function () {
  if (arguments.length == 0) return await loadGraphData.apply(this, arguments)
    ...
    if (groupNodes) {
      ...
      for (let node of p.workflow.nodes) {
        if (
          ...
          node.type.startsWith(`workflow${SEPARATOR}`)
        ) {
