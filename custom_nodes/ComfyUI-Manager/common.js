...
function internalCustomConfirm(message, confirmMessage, cancelMessage) {
  return new Promise((resolve) => {
    ...
    if (confirmMessage) confirmButton.textContent = confirmMessage
    else confirmButton.textContent = 'Confirm'
    ...
    if (cancelMessage) cancelButton.textContent = cancelMessage
    else cancelButton.textContent = 'Cancel'
...
export async function customConfirm(message) {
  try {
    let res = await window['app'].extensionManager.dialog.confirm({
      title: 'Confirm',
      message: message
    })
...
export async function customPrompt(title, message) {
  try {
    let res = await window['app'].extensionManager.dialog.prompt({
      title: title,
      message: message
    })
...
export function rebootAPI() {
  ...
  customConfirm("Are you sure you'd like to reboot the server?").then(
    (isConfirmed) => {
      if (isConfirmed) {
        try {
...
export async function migrateAPI() {
  let confirmed = await customConfirm(
    "When performing a migration, existing installed custom nodes will be renamed and the server will be restarted. Are you sure you want to apply this?\n\n(If you don't perform the migration, ComfyUI-Manager's start-up time will be longer each time due to re-checking during startup.)"
  )
  if (confirmed) {
    try {
...
export async function install_pip(packages) {
  ...
  if (res.status == 200) {
    show_message(
      `PIP package installation is processed.<br>To apply the pip packages, please click the <button id='cm-reboot-button3'><font size='3px'>RESTART</font></button> button in ComfyUI.`
    )
...
export async function install_via_git_url(url, manager_dialog) {
  ...
  if (res.status == 200) {
    show_message(
      `'${url}' is installed<BR>To apply the installed custom node, please <button id='cm-reboot-button4'><font size='3px'>RESTART</font></button> ComfyUI.`
    )
...
export const icons = {
  search:
    '<svg viewBox="0 0 24 24" width="100%" height="100%" pointer-events="none" xmlns="http://www.w3.org/2000/svg"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m21 21-4.486-4.494M19 10.5a8.5 8.5 0 1 1-17 0 8.5 8.5 0 0 1 17 0"/></svg>',
  extensions:
  ...
  conflicts:
    '<svg viewBox="0 0 400 400" width="100%" height="100%" pointer-events="none" xmlns="http://www.w3.org/2000/svg"><path fill="currentColor" d="m397.2 350.4.2-.2-180-320-.2.2C213.8 24.2 207.4 20 200 20s-13.8 4.2-17.2 10.4l-.2-.2-180 320 .2.2c-1.6 2.8-2.8 6-2.8 9.6 0 11 9 20 20 20h360c11 0 20-9 20-20 0-3.6-1.2-6.8-2.8-9.6M220 340h-40v-40h40zm0-60h-40V120h40z"/></svg>',
  passed:
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 426.667 426.667"><path fill="#6AC259" d="M213.333,0C95.518,0,0,95.514,0,213.333s95.518,213.333,213.333,213.333c117.828,0,213.333-95.514,213.333-213.333S331.157,0,213.333,0z M174.199,322.918l-93.935-93.931l31.309-31.309l62.626,62.622l140.894-140.898l31.309,31.309L174.199,322.918z"/></svg>',
  download:
    '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" width="100%" height="100%" viewBox="0 0 32 32"><path fill="currentColor" d="M26 24v4H6v-4H4v4a2 2 0 0 0 2 2h20a2 2 0 0 0 2-2v-4zm0-10l-1.41-1.41L17 20.17V2h-2v18.17l-7.59-7.58L6 14l10 10l10-10z"></path></svg>'
}
...
async function onReconnected(event) {
  if (need_restart) {
    ...
    const confirmed = await customConfirm(
      "To apply the changes to the node pack's installation status, you need to refresh the browser. Would you like to refresh?"
    )
    if (!confirmed) {
      return
    }
