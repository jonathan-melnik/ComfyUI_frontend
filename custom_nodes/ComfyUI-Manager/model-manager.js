...
export class ModelManager {
  ...
  async onQueueStatus(event) {
    ...
    if (
      ...
      event.detail.ui_target == 'model_manager'
    ) {
  ...
  async onQueueCompleted(info) {
    ...
    self.showRefresh()
    self.showMessage(
      `To apply the installed model, please click the 'Refresh' button.`,
      'red'
    )

    infoToast(
      'Tasks done',
      `[ComfyUI-Manager] All model downloading tasks in the queue have been completed.\n${info.done_count}/${info.total_count}`
    )
