...
async function switchComfyUI() {
  ...
  let res = await api.fetchApi(`/comfyui_manager/comfyui_versions`, {
    cache: 'no-store'
  })
  ...
  if (res.status == 200) {
    ...
    showVersionSelectorDialog(
      ...
      async (selected_version) => {
        ...
        let response = await api.fetchApi(
          `/comfyui_manager/comfyui_switch_version?ver=${selected_version}`,
          { cache: 'no-store' }
        )
...
async function onQueueStatus(event) {
  ...
  if (event.detail.status == 'in_progress') {
    ...
    for (let k in event.detail.nodepack_result) {
      ...
      if (v.msg == 'success') {
        success_list.push(k)
      } else if (v.msg != 'skip') failed_list.push(k)
    }
    ...
    if (success_list.length == 0 && comfyui_state.startsWith('skip')) {
      ...
      msg =
        "To apply the updates, you need to <button class='cm-small-button' id='cm-reboot-button5'>RESTART</button> ComfyUI.<hr>"
...
async function updateAll(update_comfyui) {
  ...
  if (response.status == 401) {
    customAlert(
      'Another task is already in progress. Please stop the ongoing task first.'
    )
...
class ManagerMenuDialog extends ComfyDialog {
  createControlsMid() {
    ...
    switch_comfyui_button = $el('button.cm-button', {
      type: 'button',
      textContent: 'Switch ComfyUI',
      style: {
        display: isElectron ? 'none' : 'block'
      },
      onclick: () => switchComfyUI()
    })

    restart_stop_button = $el('button.cm-button-red', {
      type: 'button',
      textContent: 'Restart',
    ...
    const res = [
      ...
      $el('button.cm-button', {
        ...
        onclick: () => {
          ...
          CustomNodesManager.instance.show(
            CustomNodesManager.ShowMode.IN_WORKFLOW
          )
        }
      }),
    ...
    let migration_btn = $el('button.cm-button-orange', {
      type: 'button',
    ...
    api
      .fetchApi('/manager/need_to_migrate')
      .then((response) => response.text())
      .then((text) => {
        if (text === 'True') {
  ...
  createControlsLeft() {
    ...
    this.datasrc_combo.setAttribute(
      'title',
    ...
    this.datasrc_combo.appendChild(
      $el('option', { value: 'remote', text: 'DB: Channel (remote)' }, [])
    )

    api
      .fetchApi('/manager/db_mode')
      .then((response) => response.text())
      .then((data) => {
        this.datasrc_combo.value = data
      })
    ...
    preview_combo.setAttribute(
      'title',
      'Configure how latent variables will be decoded during preview in the sampling process.'
    )
    ...
    component_policy_combo.setAttribute(
      'title',
      'When loading the workflow, configure which version of the component to use.'
    )
    component_policy_combo.className = 'cm-menu-combo'
    component_policy_combo.appendChild(
      $el(
        'option',
        { value: 'workflow', text: 'Component: Use workflow version' },
        []
      )
    )
    component_policy_combo.appendChild(
      $el(
        'option',
        { value: 'higher', text: 'Component: Use higher version' },
        []
      )
    )
    component_policy_combo.appendChild(
      $el('option', { value: 'mine', text: 'Component: Use my version' }, [])
    )
    api
      .fetchApi('/manager/policy/component')
      .then((response) => response.text())
      .then((data) => {
    ...
    if (isElectron) update_policy_combo.style.display = 'none'

    update_policy_combo.setAttribute(
      'title',
      'Sets the policy to be applied when performing an update.'
    )
    update_policy_combo.className = 'cm-menu-combo'
    update_policy_combo.appendChild(
      $el(
        'option',
        { value: 'stable-comfyui', text: 'Update: ComfyUI Stable Version' },
        []
      )
    )
    update_policy_combo.appendChild(
      $el(
        'option',
        { value: 'nightly-comfyui', text: 'Update: ComfyUI Nightly Version' },
        []
      )
    )
    api
      .fetchApi('/manager/policy/update')
      .then((response) => response.text())
      .then((data) => {
    ...
    return [
      ...
      $el('filedset.cm-experimental', {}, [
        ...
        $el('button.cm-experimental-button', {
          ...
          onclick: async () => {
            var url = await customPrompt(
              'Please enumerate the pip packages to be installed.\n\nExample: insightface opencv-python-headless>=4.1.1\n',
              ''
            )
    ...
    const elts = [
      ...
      $el(
        'button',
        {
          ...
          type: 'button',
          style: {
            ...(localStorage.getItem('wg_last_visited')
              ? { height: '50px' }
              : {})
          },
          onclick: (e) => {
  ...
  handleWorkflowGalleryButtonClick(e) {
    ...
    const modifyButtonStyle = (url) => {
      const workflowGalleryButton = document.getElementById(
        'workflowgallery-button'
      )
      workflowGalleryButton.style.height = '50px'
      const lastVisitedLabel = document.getElementById(
        'workflowgallery-button-last-visited-label'
      )
...
app.registerExtension({
  ...
  async setup() {
    ...
    try {
      ...
      let cmGroup = new (
        await import('../../scripts/ui/components/buttonGroup.js')
      ).ComfyButtonGroup(
        ...
        new (await import('../../scripts/ui/components/button.js')).ComfyButton(
          {
            ...
            action: () => {
              ...
              CustomNodesManager.instance.show(
                CustomNodesManager.ShowMode.FAVORITES
              )
            },
      ...
      console.log(
        'ComfyUI is outdated. New style menu based features are disabled.'
      )
    }
