...
export class CustomNodesManager {
  ...
  getActionButtons(action, rowItem, is_selected_button) {
    ...
    if (!installGroups.updatable) {
      installGroups.enabled = installGroups.enabled.filter(
        (it) => it !== 'try-update'
      )
    }
    ...
    if (rowItem?.title === 'ComfyUI-Manager') {
      installGroups.enabled = installGroups.enabled.filter(
        (it) => it !== 'disable' && it !== 'uninstall' && it !== 'switch'
      )
    }
  ...
  initGrid() {
    ...
    grid.bind('onClick', (e, d) => {
      ...
      if (btn) {
        ...
        if (
          (mode === 'install' || mode === 'switch' || mode == 'enable') &&
          item.originalData.version != 'unknown'
        ) {
  ...
  renderGrid() {
    ...
    const columns = [
      ...
      },
      {
        ...
        formatter: (title, rowItem, columnItem) => {
          ...
          else link.href = rowItem.reference
          link.target = '_blank'
      ...
      },
      {
        ...
        formatter: (extensions, rowItem, columnItem) => {
          ...
          list.push(
            `<button popovertarget="${eId}" title="${extensionsList.length} Extension Nodes" class="cn-extensions-button">${icons.extensions}</button>`
          )
          list.push(`<div popover id="${eId}" class="cn-extensions-list">`)
          list.push(
            `<h3>【${rowItem.title}】Extension Nodes (${extensionsList.length})</h3>`
          )
      ...
      },
      {
        ...
        formatter: (conflicts, rowItem, columnItem) => {
          ...
          list.push(
            `<button popovertarget="${cId}" title="${conflictsList.length} Conflicted Nodes" class="cn-conflicts-button">${icons.conflicts}</button>`
          )
          list.push(`<div popover id="${cId}" class="cn-conflicts-list">`)
          list.push(
            `<h3>【${rowItem.title}】Conflicted Nodes (${conflictsList.length})</h3>`
          )
    ...
    rows_values = rows_values.sort((a, b) => {
  ...
  async installNodeWithVersion(rowItem, btn, is_enable) {
    ...
    let res
    if (is_enable) {
      res = await api.fetchApi(`/customnode/disabled_versions/${node_id}`, {
        cache: 'no-store'
      })
    } else {
      res = await api.fetchApi(`/customnode/versions/${node_id}`, {
        cache: 'no-store'
      })
    }
    this.hideLoading()
    ...
    if (res.status == 200) {
      ...
      if (!is_enable) {
        ...
        if (
          rowItem.cnr_latest != rowItem.originalData.active_version &&
          obj.length > 0
        ) {
  ...
  async installNodes(list, btn, title, selected_version) {
    ...
    if (stats.is_processing) {
      customAlert(
        `[ComfyUI-Manager] There are already tasks in progress. Please try again after it is completed. (${stats.done_count}/${stats.total_count})`
      )
      return
    }
  ...
  async onQueueStatus(event) {
    ...
    if (
      event.detail.status == 'in_progress' &&
  ...
  async onQueueCompleted(info) {
    ...
    infoToast(
      `[ComfyUI-Manager] All node pack tasks in the queue have been completed.\n${info.done_count}/${info.total_count}`
    )
      ...
      if (!registered_nodes.has(node.type)) {
        ...
        if (node.properties.cnr_id) {
          ...
          if (item) {
            ...
            console.log(
              `CM: cannot find '${node.properties.cnr_id}' from cnr list.`
            )
  ...
  async getMissingNodesLegacy(hashMap, missing_nodes) {
    ...
    for (let k in this.custom_nodes) {
      ...
      if (unresolved_missing_nodes.has(item.id)) {
        ...
        item.files?.some((file) => unresolved_missing_nodes.has(file))
      ) {
        hashMap[item.hash] = true
      }
    }
  ...
  async getFavorites() {
    ...
    for (let k in this.custom_nodes) {
      let item = this.custom_nodes[k]
      if (item.is_favorite) hashMap[item.hash] = true
    }
  ...
  async getNodepackInWorkflow() {
    ...
    for (let k in allUsedNodes) {
      ...
      if (item) hashMap[item.hash] = true
    }
  ...
  async loadData(show_mode = ShowMode.NORMAL) {
    ...
    const skip_update =
      this.show_mode === ShowMode.UPDATE ? '' : '&skip_update=true'

    if (this.show_mode === ShowMode.UPDATE) {
      infoToast(
        'Fetching updated information. This may take some time if many custom nodes are installed.'
      )
    }
    ...
    if (this.channel !== 'default') {
      this.element.querySelector('.cn-manager-channel').innerHTML =
  ...
  setDisabled(disabled) {
    ...
    Array.from(this.element.querySelectorAll('.cn-btn-loading')).forEach(
      ($elem) => {
