...
class LoraInfoDialog extends ModelInfoDialog {
  ...
  addTags() {
    ...
    if (tags?.length) {
      ...
      if (c > MAX_TAGS) {
        ...
        hasMore = $el('p', [
          $el('span', {
            textContent: `⚠️ Only showing first ${MAX_TAGS} tags `
          }),
          $el('a', {
            href: '#',
      ...
      this.tags = $el('p', {
        textContent: '⚠️ No tag frequency metadata found'
      })
    }
  ...
  async addInfo() {
    ...
    this.addInfoEntry(
      'Base Model',
      this.metadata.ss_sd_model_name || '⚠️ Unknown'
    )
  ...
  createButtons() {
    ...
    btns.unshift(
      $el('button', {
        ...
        onclick: (e) => {
          copyTags(e, [
            ...this.tags.querySelectorAll('.pysssss-model-tag--selected')
          ])
        }
      }),
      $el('button', {
