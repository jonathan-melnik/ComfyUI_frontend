    ...
    const insertOption = (option) => {
      if (GROUPED_MENU_ORDER.hasOwnProperty(option.content)) {
        ...
        for (let i = 0; i < r.length; i++) {
          if (
            GROUPED_MENU_ORDER.hasOwnProperty(r[i].content) &&
            GROUPED_MENU_ORDER[option.content] <
              GROUPED_MENU_ORDER[r[i].content]
          ) {
            targetPos = i
            break
          }
...
export function addNode(name, nextTo, options) {
  ...
  app.graph.add(node)
  node.pos = [nextTo.pos[0] + options.shiftX, nextTo.pos[1] + options.shiftY]
  if (options.select) {
