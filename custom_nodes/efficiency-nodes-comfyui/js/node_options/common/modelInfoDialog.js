      ...
      $el(
        ...
        Object.keys(metadata).map((k) =>
          $el('div', [
            ...
            $el('span', { textContent: metadata[k] })
          ])
        )
      )
    )
...
export class ModelInfoDialog extends ComfyDialog {
  ...
  async show(type, value) {
    ...
    const req = api.fetchApi(
      '/pysssss/metadata/' + encodeURIComponent(`${type}/${value}`)
    )
    this.info = $el('div', { style: { flex: 'auto' } })
    this.img = $el('img', { style: { display: 'none' } })
    this.imgWrapper = $el('div.pysssss-preview', [this.img])
    this.main = $el('main', { style: { display: 'flex' } }, [
      this.info,
      this.imgWrapper
    ])
    this.content = $el('div.pysssss-model-content', [
      $el('h2', { textContent: this.name }),
      this.main
    ])

    const loading = $el('div', {
      textContent: 'ℹ️ Loading...',
      parent: this.content
    })
      ...
      onclick: async (e) => {
        ...
        if (textarea) {
          ...
          const resp = await api.fetchApi(
            '/pysssss/metadata/notes/' +
              encodeURIComponent(`${this.type}/${this.name}`),
            {
              method: 'POST',
  ...
  async getCivitaiDetails() {
    const req = await fetch(
      'https://civitai.com/api/v1/model-versions/by-hash/' + this.hash
    )
    if (req.status === 200) {
        ...
        if (info.images?.length) {
          ...
          this.imgSave = $el('button', {
            ...
            onclick: async () => {
              ...
              // Store it in temp
              const name =
                'temp_preview.' + new URL(this.img.src).pathname.split('.')[1]
