...
function replaceNode(oldNode, newNodeName) {
  ...
  if (!mappings[swapKey]) {
    ...
    if (reverseMapping) {
      widgetMapping = Object.entries(reverseMapping).reduce(
        (acc, [key, value]) => {
          acc[value] = key
          return acc
        },
  ...
  if (
    oldNode.type === 'KSampler (Efficient)' &&
    (newNodeName === 'KSampler Adv. (Efficient)' ||
      newNodeName === 'KSampler SDXL (Eff.)')
  ) {
    const denoise = Math.min(
      Math.max(findWidgetByName(oldNode, 'denoise').value, 0),
      1
    ) // Ensure denoise is between 0 and 1
    const steps = Math.min(
      Math.max(findWidgetByName(oldNode, 'steps').value, 0),
      10000
    ) // Ensure steps is between 0 and 10000

    const total_steps = Math.floor(steps / denoise)
    const start_at_step = total_steps - steps

    findWidgetByName(newNode, 'steps').value = Math.min(
      Math.max(total_steps, 0),
      10000
    ) // Ensure total_steps is between 0 and 10000
    findWidgetByName(newNode, 'start_at_step').value = Math.min(
      Math.max(start_at_step, 0),
      10000
    ) // Ensure start_at_step is between 0 and 10000
  } else if (
    (oldNode.type === 'KSampler Adv. (Efficient)' ||
      oldNode.type === 'KSampler SDXL (Eff.)') &&
    newNodeName === 'KSampler (Efficient)'
  ) {
    const stepsAdv = Math.min(
      Math.max(findWidgetByName(oldNode, 'steps').value, 0),
      10000
    ) // Ensure stepsAdv is between 0 and 10000
    const start_at_step = Math.min(
      Math.max(findWidgetByName(oldNode, 'start_at_step').value, 0),
      10000
    ) // Ensure start_at_step is between 0 and 10000

    const denoise = Math.min(
      Math.max((stepsAdv - start_at_step) / stepsAdv, 0),
      1
    ) // Ensure denoise is between 0 and 1
    ...
    findWidgetByName(newNode, 'steps').value = Math.min(
      Math.max(stepsTotal, 0),
      10000
    ) // Ensure stepsTotal is between 0 and 10000
  }
  ...
  oldNode.inputs.slice(oldNodeInputStartIndex).forEach((input, index) => {
    if (input && input.link !== null) {
      ...
      if (originLinkInfo) {
        ...
        if (originNode) {
          originNode.connect(
            originLinkInfo.origin_slot,
            newNode,
            index + newNodeInputStartIndex
          )
        }
      }
    }
    ...
    if (output && output.links) {
      output.links.forEach((link) => {
        ...
        if (targetLinkInfo) {
          ...
          if (targetNode) {
            newNode.connect(
              index + newNodeOutputStartIndex,
              targetNode,
              targetLinkInfo.target_slot
            )
          }
        }
      })
