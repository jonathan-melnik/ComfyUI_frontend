...
function reloadHiResFixNode(originalNode) {
  ...
  const position =
    originalNode.pos && originalNode.pos.length === 2
      ? { x: originalNode.pos[0], y: originalNode.pos[1] }
      : { x: 0, y: 0 }
  ...
  originalNode.inputs.forEach((input, index) => {
    if (input && input.link !== null) {
      ...
      if (originLinkInfo) {
        const originNode = originalNode.graph.getNodeById(
          originLinkInfo.origin_id
        )
        if (originNode) {
  ...
  originalNode.outputs.forEach((output, index) => {
    if (output && output.links) {
      output.links.forEach((link) => {
        ...
        if (targetLinkInfo) {
          const targetNode = originalNode.graph.getNodeById(
            targetLinkInfo.target_id
          )
          if (targetNode) {
...
ext.loadedGraphNode = function (node, app) {
  ...
  /*  Changes:
    ...
    */
  if (
    node.comfyClass === 'Efficient Loader' ||
    node.comfyClass === 'Eff. Loader SDXL'
  ) {
    const tokenWidget = node.widgets.find(
      (w) => w.name === 'token_normalization'
    )
    const weightWidget = node.widgets.find(
      (w) => w.name === 'weight_interpretation'
    )

    if (typeof tokenWidget.value === 'number') {
      console.log(
        '[EfficiencyUpdate]',
        `Fixing '${node.comfyClass}' token and weight widgets:`,
        node
      )
  ...
  /*  Changes:
    ...
    */
    ...
    const seedWidgetName =
      node.comfyClass === 'KSampler (Efficient)' ? 'seed' : 'noise_seed'
    const stepsWidgetName =
      node.comfyClass === 'KSampler (Efficient)' ? 'steps' : 'start_at_step'
    ...
    if (isNaN(seedWidget.value) && isNaN(stepsWidget.value)) {
      console.log(
        '[EfficiencyUpdate]',
        `Fixing '${node.comfyClass}' node widgets:`,
        node
      )
  ...
  // Check if the current node is "HighRes-Fix Script" and if any of the above fixes were applied
    ...
    const upscaleTypeWidget = node.widgets.find(
      (w) => w.name === 'upscale_type'
    )

    if (
      upscaleTypeWidget &&
      upscaleTypeWidget.value !== 'latent' &&
      upscaleTypeWidget.value !== 'pixel' &&
      upscaleTypeWidget.value !== 'both'
    ) {
      console.log(
        '[EfficiencyUpdate]',
        "Reloading 'HighRes-Fix Script' node:",
        node
      )
      ...
      const originalValues = originalNode.widgets
        .slice(0, 5)
        .map((w) => w.value)
      ...
      targetWidgetNames.forEach((name, index) => {
        ...
        if (widget && originalValues[index] !== undefined) {
          if (
            ...
            typeof originalValues[index] === 'string'
          ) {
            widget.value = originalValues[index].replace(
              'SD-Latent-Upscaler',
              'city96'
            )
          } else {
