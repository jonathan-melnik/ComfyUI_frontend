          ...
          default:
            this.serializedCtx.seedUsed =
              Math.floor(Math.random() * range) *
                (this.seedWidget.options.step / 10) +
              min
            break
        }

        // Ensure the seed value is an integer and remains within the accepted range
        this.serializedCtx.seedUsed = Number.isInteger(
          this.serializedCtx.seedUsed
        )
          ? Math.min(Math.max(this.serializedCtx.seedUsed, min), max)
    ...
    if (NODE_WIDGET_MAP[nodeData.name]) {
      addMenuHandler(nodeType, function (insertOption) {
        ...
        if (nodeData.name === 'Noise Control Script') {
          ...
          const addSeedNoiseWidget = this.widgets.find(
            (w) => w.name === 'add_seed_noise'
          )
          ...
          const useSameSeedWidget = this.widgets.find(
            (w) => w.name === 'use_same_seed'
          )
