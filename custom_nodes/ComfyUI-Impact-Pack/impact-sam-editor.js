...
async function uploadMask(filepath, formData) {
  ...
  ComfyApp.clipspace.imgs[ComfyApp.clipspace['selectedIndex']].src =
    `view?filename=${filepath.filename}&type=${filepath.type}`
...
class ImpactSamEditorDialog extends ComfyDialog {
  ...
  setlayout(imgCanvas, maskCanvas, pointsCanvas) {
    ...
    var confidence_slider = this.createLeftSlider(
      self,
      'Confidence',
      (event) => {
    ...
    var clearButton = this.createLeftButton('Clear', () => {
      ...
      self.pointsCtx.clearRect(
        0,
        0,
        self.pointsCanvas.width,
        self.pointsCanvas.height
      )
    ...
    var detectButton = this.createLeftButton('Detect', () =>
      self.detect_and_invalidate_mask_canvas(self)
    )

    var cancelButton = this.createRightButton('Cancel', () => {
      document.removeEventListener(
        'mouseup',
        ImpactSamEditorDialog.handleMouseUp
      )
      document.removeEventListener(
        'keydown',
        ImpactSamEditorDialog.handleKeyDown
      )
      self.close()
    })
  ...
  show() {
    ...
    this.message_box = $el('p', [
      'Please wait a moment while the SAM model and the image are being loaded.'
    ])
    this.element.appendChild(this.message_box)

    if (self.imgCtx) {
      self.imgCtx.clearRect(
        0,
        0,
  ...
  setImages(target_image_path, imgCanvas, pointsCanvas) {
    ...
    window.addEventListener('resize', () => {
      ...
      pointsCanvas.height =
        (drawHeight * imgCanvas.clientHeight) / imgCanvas.height
  ...
  setEventHandler(targetCanvas) {
    ...
    targetCanvas.addEventListener('pointermove', (event) =>
      this.updateBrushPreview(self, event)
    )
    targetCanvas.addEventListener('pointerdown', (event) =>
      this.handlePointerDown(self, event)
    )
    targetCanvas.addEventListener('pointerover', (event) => {
  ...
  invalidatePointsCanvas(self) {
    ...
    for (const i in self.prompt_points) {
      ...
      if (is_positive) ctx.fillStyle = 'blue'
  ...
  async detect(self) {
    ...
    for (const i in self.prompt_points) {
      ...
      } else negative_points.push(point)
    }
  ...
  async save(self) {
    ...
    const save_data = save_ctx.getImageData(
      0,
      0,
      save_canvas.width,
      save_canvas.height
