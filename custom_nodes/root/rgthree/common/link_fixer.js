...
export function fixBadLinks(
  ...
  async function patchNodeSlot(node, ioDir, slot, linkId, op) {
    ...
    if (ioDir == IoDirection.INPUT) {
      ...
      patchedNode['outputs'][slot] = patchedNode['outputs'][slot] || {
        links: [
          ...(((_b =
            (_a = node.outputs) === null || _a === void 0
              ? void 0
      ...
      if (op === 'ADD') {
        ...
        if (fix) {
          ;(_c = node.outputs) === null || _c === void 0
            ? void 0
            : _c[slot].links.splice(linkIdIndex, 1)
        }
      }
    }
  ...
  function nodeHasLinkId(node, ioDir, slot, linkId) {
    ...
    if (ioDir === IoDirection.INPUT) {
      let nodeHasIt =
        ((_b =
          (_a = node.inputs) === null || _a === void 0 ? void 0 : _a[slot]) ===
          null || _b === void 0
          ? void 0
          : _b.link) === linkId
      if (
        (_c = patchedNodeSlots[node.id]) === null || _c === void 0
          ? void 0
          : _c['inputs']
      ) {
      ...
      let nodeHasIt =
        (_f =
          (_e =
            (_d = node.outputs) === null || _d === void 0
              ? void 0
              : _d[slot]) === null || _e === void 0
            ? void 0
            : _e.links) === null || _f === void 0
          ? void 0
          : _f.includes(linkId)
      if (
        (_j =
          (_h =
            (_g = patchedNodeSlots[node.id]) === null || _g === void 0
              ? void 0
              : _g['outputs']) === null || _h === void 0
            ? void 0
            : _h[slot]) === null || _j === void 0
          ? void 0
          : _j['changes'][linkId]
      ) {
        let patchedHasIt =
          (_k = patchedNodeSlots[node.id]['outputs'][slot]) === null ||
          _k === void 0
            ? void 0
            : _k.links.includes(linkId)
        if (fix && nodeHasIt !== patchedHasIt) {
          throw Error('Error. Expected node to match patched data.')
        }
        has = !!patchedHasIt
  ...
  for (let l of linksReverse) {
    ...
    const patchOrigin = (op, id = link.id) =>
    ...
    const patchTarget = (op, id = link.id) =>
    ...
    if (targetHasLink() || originHasLink()) {
      if (!originHasLink()) {
        ...
        !silent &&
          logger.log(
            `${link.id} is funky... ${targetLog} is NOT correct (is ${(_b = (_a = targetNode.inputs) === null || _a === void 0 ? void 0 : _a[link.target_slot]) === null || _b === void 0 ? void 0 : _b.link}), but ${originLog} contains it`
          )
        if (!targetHasAnyLink()) {
          !silent &&
            logger.log(
  ...
  for (let l of linksReverse) {
    ...
    if (
      ...
      !silent &&
        logger.log(
          `${link.id} is def invalid; BOTH origin node ${link.origin_id} ${!originNode ? 'is removed' : `doesn\'t have ${link.id}`} and ${link.origin_id} target node ${!targetNode ? 'is removed' : `doesn\'t have ${link.id}`}.`
        )
    ...
    for (let i = data.deletedLinks.length - 1; i >= 0; i--) {
      ...
      if (graph.getNodeById) {
        ...
        const idx = graph.links.findIndex(
          (l) =>
            l &&
            (l[0] === data.deletedLinks[i] || l.id === data.deletedLinks[i])
        )
        if (idx === -1) {
  ...
  !silent &&
    logger.log(
      `${fix ? 'Made' : 'Would make'} ${data.patchedNodes.length || 'no'} node link patches, and ${data.deletedLinks.length || 'no'} stale link removals.`
    )
