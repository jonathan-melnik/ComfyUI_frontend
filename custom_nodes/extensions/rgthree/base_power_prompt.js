...
export class PowerPrompt {
  constructor(node, nodeData) {
    ...
    this.node.configure = (info) => {
      ...
      oldConfigure === null || oldConfigure === void 0
        ? void 0
    ...
    this.node.onConnectionsChange = (
      ...
      oldOnConnectionsChange === null || oldOnConnectionsChange === void 0
        ? void 0
        : oldOnConnectionsChange.apply(this.node, [
            type,
            slotIndex,
            isConnected,
            link_info,
    ...
    this.node.onConnectOutput = (
      ...
      if (oldOnConnectOutput) {
        canConnect =
          oldOnConnectOutput === null || oldOnConnectOutput === void 0
            ? void 0
            : oldOnConnectOutput.apply(this.node, [
                outputIndex,
                inputType,
                inputSlot,
                inputNode,
    ...
    this.node.onPropertyChanged = (property, value, prevValue) => {
      onPropertyChanged &&
        onPropertyChanged.call(this, property, value, prevValue)
  ...
  stabilizeInputsOutputs() {
    ...
    const clipLinked = this.node.inputs.some(
      (i) => i.name.includes('clip') && !!i.link
    )
    const modelLinked = this.node.inputs.some(
      (i) => i.name.includes('model') && !!i.link
    )
  ...
  shouldRemoveServerWidget(widget) {
    var _a, _b, _c, _d
    return (
      ((_a = widget.name) === null || _a === void 0
        ? void 0
        : _a.startsWith('insert_')) ||
      ((_b = widget.name) === null || _b === void 0
        ? void 0
        : _b.startsWith('target_')) ||
      ((_c = widget.name) === null || _c === void 0
        ? void 0
        : _c.startsWith('crop_')) ||
      ((_d = widget.name) === null || _d === void 0
        ? void 0
        : _d.startsWith('values_'))
    )
  }
  refreshCombos(nodeData) {
    ...
    let filter = null
    if (
      (_a = this.node.properties['combos_filter']) === null || _a === void 0
        ? void 0
        : _a.trim()
    ) {
      try {
    ...
    let data = Object.assign(
      {},
      ((_b = this.nodeData.input) === null || _b === void 0
        ? void 0
        : _b.optional) || {},
      ((_c = this.nodeData.input) === null || _c === void 0
        ? void 0
        : _c.hidden) || {}
    )
    for (const [key, value] of Object.entries(data)) {
      if (Array.isArray(value[0])) {
        ...
        if (key.startsWith('insert')) {
          ...
          const shouldShow =
            values.length > 2 ||
            (values.length > 1 && !values[1].match(/^disable\s[a-z]/i))
          if (shouldShow) {
            if (!this.combos[key]) {
              this.combos[key] = this.node.addWidget(
                ...
                (selected) => {
                  if (
                    selected !== values[0] &&
                    !selected.match(/^disable\s[a-z]/i)
                  ) {
                    wait().then(() => {
              ...
              this.combos[key].computeSize = function (width) {
                var _a, _b
                const size = ((_b = (_a = this).oldComputeSize) === null ||
                _b === void 0
                  ? void 0
                  : _b.call(_a, width)) || [width, LiteGraph.NODE_WIDGET_HEIGHT]
            ...
            this.node.widgets.splice(
              this.node.widgets.indexOf(this.combos[key]),
              1
            )
  ...
  insertSelectionText(text) {
    ...
    let first = prompt
      .substring(0, this.promptEl.selectionEnd)
      .replace(/ +$/, '')
    first =
      first +
  ...
  addAndHandleKeyboardLoraEditWeight() {
    this.promptEl.addEventListener('keydown', (event) => {
      ...
      let weight =
        (_b = Number(
          (_a = selectedText.match(/:(-?\d*(\.\d*)?)>$/)) === null ||
            _a === void 0
            ? void 0
            : _a[1]
        )) !== null && _b !== void 0
  ...
  patchNodeRefresh() {
    ...
    this.node.onRemoved = () => {
      oldNodeRemoved === null || oldNodeRemoved === void 0
        ? void 0
