...
function findMatchingIndexByTypeOrName(otherNode, otherSlot, ctxSlots) {
  ...
  const otherSlotName = otherSlot.name
    .toUpperCase()
    .replace('OPT_', '')
    .replace('_NAME', '')
  let ctxSlotIndex = -1
  if (
    ['CONDITIONING', 'INT', 'STRING', 'FLOAT', 'COMBO'].includes(otherSlotType)
  ) {
    ctxSlotIndex = ctxSlots.findIndex((ctxSlot) => {
      const ctxSlotName = ctxSlot.name
        .toUpperCase()
        .replace('OPT_', '')
        .replace('_NAME', '')
      ...
      if (
        ...
        (ctxSlotName === 'STEP_REFINER' &&
          otherSlotName.includes('REFINER_STEP'))
      ) {
        return true
      }
      if (
        (otherNodeType.includes('POSITIVE') ||
          otherNodeName.includes('POSITIVE')) &&
      ...
      if (
        (otherNodeType.includes('NEGATIVE') ||
          otherNodeName.includes('NEGATIVE')) &&
...
export class BaseContextNode extends RgthreeBaseServerNode {
  ...
  set _collapsed_width(width) {
    ...
    this.___collapsed_width =
      30 + (title ? 10 + ctx.measureText(title).width : 0)
    ctx.font = oldFont
  }
  connectByType(slot, sourceNode, sourceSlotType, optsIn) {
    ...
    if (!canConnect && slot === 0) {
      ...
      for (const [index, input] of (sourceNode.inputs || []).entries()) {
        ...
        const thisOutputSlot = findMatchingIndexByTypeOrName(
          sourceNode,
          input,
          this.outputs
        )
  ...
  connectByTypeOutput(slot, sourceNode, sourceSlotType, optsIn) {
    ...
    let canConnect =
      ...
      super.connectByTypeOutput.call(
        this,
        slot,
        sourceNode,
        sourceSlotType,
    ...
    if (!canConnect && slot === 0) {
      ...
      for (const [index, output] of (sourceNode.outputs || []).entries()) {
        if (
          ((_a = output.links) === null || _a === void 0
            ? void 0
            : _a.length) &&
          !ctrlKey
        ) {
        ...
        const thisInputSlot = findMatchingIndexByTypeOrName(
          sourceNode,
          output,
          this.inputs
        )
  ...
  static setUp(comfyClass, nodeData, ctxClass) {
    ...
    wait(500).then(() => {
      LiteGraph.slot_types_default_out['RGTHREE_CONTEXT'] =
        LiteGraph.slot_types_default_out['RGTHREE_CONTEXT'] || []
      LiteGraph.slot_types_default_out['RGTHREE_CONTEXT'].push(
        comfyClass.comfyClass
      )
    })
  }
...
class BaseContextMultiCtxInputNode extends BaseContextNode {
  ...
  addContextInput(num = 1) {
    for (let i = 0; i < num; i++) {
      this.addInput(
        `ctx_${String(this.inputs.length + 1).padStart(2, '0')}`,
        'RGTHREE_CONTEXT'
      )
    }
  }
