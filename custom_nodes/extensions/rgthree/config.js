...
const CONFIGURABLE = {
  ...
  menus: [
    ...
    },
    {
      ...
      description:
        'Will list bookmarks in the rgthree-comfy right-click context menu.'
    }
  ],
  groups: [
    {
      ...
      description:
        "Show quick toggles in Groups' Headers to quickly mute, bypass or queue.",
      subconfig: [
        {
      ...
      input = $el(`select[id="${item.key}"]`, {
        ...
        children: item.options.map((o) => {
          ...
          return $el(`option[value="${valueSerialized}"]`, {
            text: label,
            selected:
              valueSerialized === JSON.stringify({ value: initialValue })
          })
        })
      })
    }
...
export class RgthreeConfigDialog extends RgthreeDialog {
  constructor() {
    ...
    content.appendChild(
      RgthreeConfigDialog.buildFieldset(CONFIGURABLE['features'], 'Features')
    )
    content.appendChild(
      RgthreeConfigDialog.buildFieldset(CONFIGURABLE['menus'], 'Menus')
    )
    content.appendChild(
      RgthreeConfigDialog.buildFieldset(CONFIGURABLE['groups'], 'Groups')
    )
    content.appendChild(
      RgthreeConfigDialog.buildFieldset(CONFIGURABLE['advanced'], 'Advanced')
    )
    ...
    const dialogOptions = {
      ...
      onBeforeClose: () => {
        ...
        if (Object.keys(changed).length) {
          return confirm(
            'Looks like there are unsaved changes. Are you sure you want close?'
          )
        }
        return true
      },
          ...
          callback: async (e) => {
            ...
            if (success) {
              for (const key of Object.keys(changed)) {
                ;(_b =
                  (_a = Object.values(CONFIGURABLE)
                    .flat()
                    .find((f) => f.key === key)) === null || _a === void 0
                    ? void 0
                    : _a.onSave) === null || _b === void 0
                  ? void 0
                  : _b.call(_a, changed[key])
              }
              this.close()
              rgthree.showMessage({
                id: 'config-success',
  ...
  getChangedFormData() {
    return $$('[data-name]', this.contentElement).reduce((acc, el) => {
      ...
      if (type === String(ConfigType.BOOLEAN)) {
        ...
        currentValue =
          currentValueEl === null || currentValueEl === void 0
            ? void 0
            : currentValueEl.value
        if (currentValueEl.nodeName === 'SELECT') {
          currentValue = JSON.parse(currentValue).value
        } else if (
          currentValueEl.classList.contains('rgthree-checklist-group')
        ) {
          currentValue = []
  ...
  type: () => {
    return $el('tr.rgthree-comfyui-settings-row', {
      children: [
        ...
        $el('td', {
          child: $el(
            'button.rgthree-button.-blue[text="rgthree-comfy settings"]',
            {
              events: {
                click: (e) => {
