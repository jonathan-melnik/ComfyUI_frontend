...
export class YouMLShareDialog extends ComfyDialog {
  ...
  createLayout() {
    ...
    const buttonsSection = $el('div.action-button-panel', {}, [
        ...
        if (id) {
          ...
          if (this.selectedFile) {
            try {
              ...
              messagePrefix =
                'Workflow has been shared, but thumbnail upload failed. You can create a thumbnail on YouML later.'
            }
          }
          this.message.innerHTML =
            `${messagePrefix} To turn your workflow into an interactive app, ` +
  ...
  async show(potentialOutputs, potentialOutputNodes) {
    ...
    if (this.selectedNodeId) {
      const index = potentialOutputNodes.findIndex(
    ...
    const newRadioButtons = $el(
      ...
      potentialOutputs.map((output, index) => {
        ...
        return $el(
          `label.output-label${radioButton.checked ? '.checked' : ''}`,
          {},
    ...
    if (this.radioButtons.length === 0) {
      header = $el(
        'div.missing-output-message',
        {
          textContent: 'Queue Prompt to see the outputs and select a thumbnail'
        },
        []
      )
    } else {
      header = $el(
        'div.select-output-message',
        { textContent: 'Choose one from the outputs (scroll to see all)' },
        []
      )
    }
