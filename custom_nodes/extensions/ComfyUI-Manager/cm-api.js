...
import { customAlert, customConfirm, sleep } from './common.js'
...
async function tryInstallCustomNode(event) {
  ...
  if (event.detail.target.installed == 'Disabled') {
    msg +=
      'However, the extension is currently disabled. Would you like to enable it and reboot?'
  } else if (event.detail.target.installed == 'True') {
    msg +=
      'However, it seems that the extension is in an import-fail state or is not compatible with the current version. Please address this issue.'
  } else {
