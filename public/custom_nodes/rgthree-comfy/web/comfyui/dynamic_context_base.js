...
export class DynamicContextNodeBase extends BaseContextNode {
  ...
  isOwnedInput(inputOrName) {
    const name =
      typeof inputOrName == 'string'
        ? inputOrName
        : (inputOrName === null || inputOrName === void 0
            ? void 0
            : inputOrName.name) || ''
  ...
  handleUpstreamMutation(mutation) {
    ...
    if (mutation.operation === InputMutationOperation.ADDED) {
      ...
      if (!slot) {
        throw new Error(
          'Cannot have an ADDED mutation without a provided slot data.'
        )
      }
    ...
    if (mutation.operation === InputMutationOperation.REMOVED) {
      ...
      if (!slot) {
        throw new Error(
          'Cannot have an REMOVED mutation without a provided slot data.'
        )
      }
    ...
    if (mutation.operation === InputMutationOperation.RENAMED) {
      ...
      if (!slot) {
        throw new Error(
          'Cannot have an RENAMED mutation without a provided slot data.'
        )
      }
  ...
  onNodeCreated() {
    ...
    this.addCustomWidget(
      new RgthreeInvisibleWidget(
        'output_keys',
        'RGTHREE_DYNAMIC_CONTEXT_OUTPUTS',
        '',
        () => {
          return (node.outputs || [])
            .map((o, i) => i > 0 && o.name)
  ...
  renameContextInput(index, newName, forceOwnBool = null) {
    ...
    newName = this.stripOwnedPrefix(
      newName.trim() || this.getSlotDefaultInputLabel(index)
    )
    if (
      forceOwnBool === true ||
      (this.isOwnedInput(oldName) && forceOwnBool !== false)
    ) {
  ...
  fixInputsOutputsLinkSlots() {
    if (!this.hasShadowInputs) {
      ...
      for (let index = inputs.length - 1; index > 0; index--) {
        const input = inputs[index]
        if (
          (input === null || input === void 0 ? void 0 : input.link) != null
        ) {
