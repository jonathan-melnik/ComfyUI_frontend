...
export class DynamicContextNode extends DynamicContextNodeBase {
  ...
  onConnectionsChange(type, slotIndex, isConnected, link, ioSlot) {
    var _a
    ;(_a = super.onConnectionsChange) === null || _a === void 0
      ? void 0
      : _a.call(this, type, slotIndex, isConnected, link, ioSlot)
    if (this.configuring) {
      return
    }
  ...
  handleInputConnected(slotIndex) {
    ...
    if (slotIndex === 0) {
      let baseNodeInfos = getConnectedInputInfosAndFilterPassThroughs(
        this,
        this,
        0
      )
      ...
      if (
        baseNodesDynamicCtx === null || baseNodesDynamicCtx === void 0
          ? void 0
          : baseNodesDynamicCtx.provideInputsData
      ) {
        const inputsData =
          CONTEXT_SERVICE.getDynamicContextInputsData(baseNodesDynamicCtx)
  ...
  handleNewInputConnected(slotIndex) {
    if (!this.isInputSlotForNewInput(slotIndex)) {
      throw new Error(
        'Expected the incoming slot index to be the "new input" input.'
      )
    }
    const ioSlot = this.inputs[slotIndex]
    let cxn = null
    ...
    if (
      (cxn === null || cxn === void 0 ? void 0 : cxn.type) &&
      (cxn === null || cxn === void 0 ? void 0 : cxn.name)
    ) {
      let name = this.addOwnedPrefix(
        this.getNextUniqueNameForThisNode(cxn.name)
      )
      ...
      for (let index = inputs.length - 1; index > 0; index--) {
        ...
        if (!this.isOwnedInput(input.name)) {
          if (
            input.link ||
            ((_b =
              (_a = this.outputs[index]) === null || _a === void 0
                ? void 0
  ...
  getNextUniqueNameForThisNode(desiredName) {
    const inputs = this.getContextInputsList()
    const allExistingKeys = inputs.map((i) =>
      this.stripOwnedPrefix(i.name).toLocaleUpperCase()
    )
  ...
  stabilizeNames() {
    ...
    for (const [index, input] of inputs.entries()) {
      ...
      if (!this.isOwnedInput(name)) {
        ...
        while (
          names.includes(this.stripOwnedPrefix(name).toLocaleUpperCase())
        ) {
    ...
    return [
      {
        ...
        callback: () => {
          ...
          var inner = () => {
            this.handleContextMenuRenameInputDialog(
              slot.slot,
              dialogInput.value
            )
            dialog.close()
