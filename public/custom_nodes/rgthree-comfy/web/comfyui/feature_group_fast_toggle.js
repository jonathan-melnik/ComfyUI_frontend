...
function getToggles() {
  return [
    ...CONFIG_SERVICE.getFeatureValue('group_header_fast_toggle.toggles', [])
  ].reverse()
}
  ...
  async setup() {
    setInterval(() => {
      if (
        ...
        CONFIG_SERVICE.getFeatureValue('group_header_fast_toggle.show') !==
          'always'
      ) {
    ...
    rgthree.addEventListener('on-process-mouse-down', (e) => {
      ...
      if (canvas.selected_group) {
        ...
        const clickedOnToggle =
          clickedOnToggleButton(originalEvent, group) || ''
        const toggleAction =
          clickedOnToggle === null || clickedOnToggle === void 0
            ? void 0
            : clickedOnToggle.toLocaleUpperCase()
        if (toggleAction) {
          if (toggleAction === 'QUEUE') {
            const outputNodes = getOutputNodes(group._nodes)
            if (
              !(outputNodes === null || outputNodes === void 0
                ? void 0
                : outputNodes.length)
            ) {
              rgthree.showMessage({
            ...
            if (toggleMode) {
              ...
              const isAllMuted =
                !hasAnyActiveNodes &&
                group._nodes.every((n) => n.mode === LiteGraph.NEVER)
              const isAllBypassed =
                !hasAnyActiveNodes &&
                !isAllMuted &&
                group._nodes.every((n) => n.mode === 4)
          ...
          if (toggle === 'queue') {
            ...
            if (toggle === 'mute') {
              ...
              if (on) {
                ...
                ctx.stroke(
                  new Path2D(
                    `${eyeFrame(midX, midY)} ${eyeFrame(midX, midY, -1)}`
                  )
                )
                ctx.globalAlpha = this.editor_alpha * 0.5
                ctx.stroke(
                  new Path2D(
                    `${eyeLashes(midX, midY)} ${eyeLashes(midX, midY, -1)}`
                  )
                )
              ...
              ctx.fill(
                new Path2D(
                  `${circlePath(x + BTN_GRID * 3, midY, BTN_GRID * 1.8)}`
                )
              )
            }
          }
