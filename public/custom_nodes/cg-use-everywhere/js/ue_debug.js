...
export function add_debug() {
  ...
  defineProperty(app.canvas, 'dirty_canvas', {
    ...
    set: (v) => {
      dirty_canvas = v
    } // a breakpoint here catches the calls that mark the canvas as dirty
  })

  var dirty_bg_canvas = true
  defineProperty(app.canvas, 'dirty_bg_canvas', {
    get: () => {
      return dirty_bg_canvas
    },
    set: (v) => {
      dirty_bg_canvas = v
    } // a breakpoint here catches the calls that mark the background canvas as dirty
  })
}
