...
function display_name(node) {
  ...
  if (node?.properties['Node name for S&R'])
    return node.properties['Node name for S&R']
...
class UseEverywhere {
  constructor() {
    ...
    if (this.title_regex)
      this.description += ` - node title regex '${this.title_regex.source}'`
    if (this.input_regex)
      this.description += ` - input name regex '${this.input_regex.source}'`
  }
  ...
  sending_differs_from(another_ue) {
    ...
    for (var i = 0; i < this.sending_to.length; i++) {
      if (
        this.sending_to[i].node.id != another_ue.sending_to[i].node.id ||
        this.sending_to[i].input_index != another_ue.sending_to[i].input_index
      )
        return true
    }
    return false
  ...
  matches(node, input) {
    ...
    if (node.type == 'Highway' && typeof this.input_regex === 'string') {
      // Highway nodes - broken if there are two matches...
      ...
      if (input_label_split.length == 1) {
        ...
        if (
          input_label_split[0] == this.input_regex &&
          input_label_split[1] == input.type
        )
          return true
        return false
      }
...
function validity_errors(params) {
  if (!node_is_live(params.controller))
    return `UE node ${params.output[0]} is not alive`
  if (!node_is_live(get_real_node(params.output[0])))
    return `upstream node ${params.output[0]} is not alive`
  return ''
}
...
class UseEverywhereList {
  ...
  add_ue(
    ...
    if (group_regex) {
      params.restrict_to = nodes_in_groups_matching(
        group_regex,
        params.restrict_to
      )
    }
    if (real_node.properties['priority_boost'])
      params.priority += real_node.properties['priority_boost']
    ...
    if (error === '') {
      ...
      Logger.log(
        Logger.PROBLEM,
        `Rejected ${ue.description} because ${error}`,
        params
      )
    }
  }
  ...
  find_best_match(node, input, _ambiguity_messages) {
    ...
    var matches = this.ues.filter((candidate) => candidate.matches(node, input))
    if (matches.length == 0) {
      Logger.log(
        Logger.INFORMATION,
        `'${display_name(node)}' optional input '${input.name}' unmatched`
      )
      return undefined
    }
    if (matches.length > 1) {
    ...
    Logger.log(
      Logger.INFORMATION,
      `'${display_name(node)}' input '${input.name}' matched to ${matches[0].description}`
    )
    return matches[0]
  }
  ...
  all_ue_connections_for(node_id) {
    ...
    this.ues.forEach((ue) => {
      ue.sending_to.forEach((st) => {
        if (
          get_real_node(st.node.id).id == node_id ||
          get_real_node(ue.controller.id).id == node_id
        ) {
          ue_connections.push({
            type: ue.type,
            input_index: st.input_index,
