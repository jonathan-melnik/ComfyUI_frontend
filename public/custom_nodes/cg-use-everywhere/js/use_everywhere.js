...
import {
  canvas_menu_settings,
  main_menu_settings,
  node_menu_settings,
  non_ue_menu_settings
} from './use_everywhere_settings.js'
import {
  UpdateBlocker,
  displayMessage,
...
function inject_outdating_into_objects(array, methodname, tracetext) {
  if (array) {
    array.forEach((object) => {
      inject_outdating_into_object_method(object, methodname, tracetext)
    })
  }
}
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    ...
    nodeType.prototype.onConnectionsChange = function (
      ...
      if (this.IS_UE && side == 1) {
        ...
        if (this.type == 'Anything Everywhere?' && slot != 0) {
          ...
          const type =
            connect && link_info
              ? get_real_node(link_info?.origin_id)?.outputs[
    ...
    nodeType.prototype.getExtraMenuOptions = function (_, options) {
      ...
      inject_outdating_into_objects(
        options,
        'callback',
        `menu option on ${this.id}`
      )
    }
      ...
      nodeType.prototype.onNodeCreated = function () {
        ...
        if (this.inputs) {
          ...
          for (const input of this.inputs) {
            if (
              input.widget &&
              !this.widgets.find((w) => w.name === input.widget.name)
            )
  ...
  async nodeCreated(node) {
    ...
    if (!node.__bgcolor) {
      ...
      defineProperty(node, 'bgcolor', {
        get: () => {
          return node.__bgcolor
        },
        set: (v) => {
          node.__bgcolor = v
          node.afterChangeMade?.('bgcolor', v)
        }
      })
    }
    ...
    if (node.IS_UE) {
      ...
      node.onDrawTitleBar = function (ctx, title_height) {
        ...
        if (
          node.properties.group_restricted ||
          node.properties.color_restricted
        )
    ...
    if (is_helper(node)) {
      // editing a helper node makes the list dirty
      inject_outdating_into_objects(
        node.widgets,
        'callback',
        `widget callback on ${this.id}`
      )
    }

    // removing a node makes the list dirty
    inject_outdating_into_object_method(
      node,
      'onRemoved',
  ...
  loadedGraphNode(node) {
    if (node.flags.collapsed && node.loaded_when_collapsed)
      node.loaded_when_collapsed()
  },
  ...
  async setup() {
    ...
    api.addEventListener('status', ({ detail }) => {
      if (linkRenderController)
        linkRenderController.note_queue_size(
          detail ? detail.exec_info.queue_remaining : 0
        )
    })
    ...
    /*
        ...
        */
    const _original_save_onclick =
      document.getElementById('comfy-save-button').onclick
    document.getElementById('comfy-save-button').onclick = function () {
      graphAnalyser.pause()
      _original_save_onclick()
      graphAnalyser.unpause()
    }
    const _original_save_api_onclick = document.getElementById(
    ...
    /* 
        ...
        */
    const original_getCanvasMenuOptions =
    ...
    defineProperty(app.canvas, 'allow_searchbox', {
      get: function () {
        if (this.use_original_menu) {
          return original_allow_searchbox
        }
        if (
          app.ui.settings.getSettingValue('AE.replacesearch') &&
