...
function update_picklist(node, inputname) {
  ...
  let options = []
  if (inputname == 'title_regex') {
    options =
      LinkRenderController.instance().ue_list?.all_nodes_with_unmatched_input(
        node.input_type[0]
      )
  } else if (inputname == 'input_regex') {
    options =
      LinkRenderController.instance().ue_list?.all_unmatched_input_names(
        node.input_type[0]
      )
  } else if (inputname == 'group_regex') {
    options = LinkRenderController.instance().ue_list?.all_group_names(
      node.input_type[0]
    )
  }
...
function getClipPath(node, element) {
  ...
  app.graph._nodes.forEach((other_node) => {
    if (other_node.id == node.id) {
      ...
      const intersection = intersect(
        {
          x: widgetRect.x / scale,
          y: widgetRect.y / scale,
  ...
  const path = clip
    ? `polygon(0% 0%, 0% 100%, ${clip.x}px 100%, ${clip.x}px ${clip.y}px, ${clip.x + clip.width}px ${clip.y}px, ${clip.x + clip.width}px ${clip.y + clip.height}px, ${clip.x}px ${clip.y + clip.height}px, ${clip.x}px 100%, 100% 100%, 100% 0%)`
    : ''
  return path
}
  ...
  const widget = node.addDOMWidget(inputname, 'input', label, {
    ...
    onDraw(w) {
      ...
      if (Object.values(app.canvas.selected_nodes)[0]?.id == node.id) {
        ...
        w.element.style.clipPath = null
        w.element.style.willChange = null
      } else {
...
function add_autoprompts() {
  ...
  ComfyWidgets.STRING = function (node, inputName, inputData, app) {
    if (
      true ||
      !is_UEnode(node) ||
      !inputName?.includes('regex') ||
      !app.ui.settings.getSettingValue('AE.autoprompt')
    ) {
