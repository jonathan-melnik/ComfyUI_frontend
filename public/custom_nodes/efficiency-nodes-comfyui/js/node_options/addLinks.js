...
function createKSamplerEntry(
  ...
  const contentLabel = subNodeNickname
    ? `${kSamplerNickname} + ${subNodeNickname}`
    : kSamplerNickname

  return {
    content: contentLabel,
    callback: function () {
      const kSamplerNode = addNode(kSamplerLabel, node, {
        shiftX: node.size[0] + 50
      })
...
function getMenuValues(type, node) {
  ...
  const excludedSubNodeTypes = [
    'NoiseControl',
    'HiResFix',
    'TiledUpscale',
    'AnimateDiff'
  ] // Nodes to exclude from the menu
  ...
  for (const subNodeType of subNodeTypes) {
    ...
    if (!excludedSubNodeTypes.includes(subNodeType)) {
      const menuEntry = createKSamplerEntry(
        node,
        type === 'Efficient' ? 'Eff' : 'SDXL',
        subNodeType,
        type === 'SDXL'
      )
      ...
      addMenuHandler(nodeType, function (insertOption) {
        insertOption({
          ...
          callback: (value, options, e, menu, node) =>
            showAddLinkMenuCommon(value, options, e, menu, node, linkType)
        })
      })
    }
  }
