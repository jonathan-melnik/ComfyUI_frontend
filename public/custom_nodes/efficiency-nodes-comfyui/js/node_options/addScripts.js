...
function addAndConnectScriptNode(scriptType, selectedNode) {
  ...
  if (
    newNodeType[0].includes('input') &&
    selectedNodeType[0].includes('output')
  ) {
    newNode.pos[0] += selectedNode.size[0] + 50
  } else if (
    newNodeType[0].includes('output') &&
    selectedNodeType[0].includes('input')
  ) {
      ...
      if (newNodeType[0] === 'input & output') {
        ...
        if (connectedNodes && connectedNodes.length) {
          for (let connectedNode of connectedNodes) {
            ...
            const targetSlot =
              connectedNode.type in connectionMap
                ? connectionMap[connectedNode.type][1]
                : 0
