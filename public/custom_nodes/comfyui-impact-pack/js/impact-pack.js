...
class ImpactProgressBadge {
  ...
  addStatusHandler(nodeType) {
    ...
    nodeType.prototype.onDrawForeground = function (ctx) {
      ...
      const { fgColor, bgColor, text, progress, progressColor } = {
        ...state.status
      }
...
function progressExecuteHandler(event) {
  if (event.detail?.output?.aux) {
    ...
    if (input_tracking.hasOwnProperty(id)) {
      if (
        input_tracking.hasOwnProperty(id) &&
        input_tracking[id][0] != event.detail.output.aux[0]
      ) {
        input_dirty[id] = true
      } else {
      }
  ...
  if (event.detail.images.length > 0) {
    ...
    for (let i in nodes) {
      if (nodes[i].type == 'ImageReceiver') {
        ...
        if (nodes[i].widgets[1].type == 'converted-widget') {
          for (let j in nodes[i].inputs) {
            ...
            if (input.name === 'link_id') {
              if (input.link) {
                let src_node =
                  app.graph._nodes_by_id[app.graph.links[input.link].origin_id]
                if (
                  src_node.type == 'ImpactInt' ||
                  src_node.type == 'PrimitiveNode'
                ) {
                  is_linked = true
                }
              }
        ...
        if (is_linked) {
          if (data.subfolder)
            nodes[i].widgets[0].value =
              `${data.subfolder}/${data.filename} [${data.type}]`
          else nodes[i].widgets[0].value = `${data.filename} [${data.type}]`
          ...
          img.src =
            `/view?filename=${data.filename}&type=${data.type}&subfolder=${data.subfolder}` +
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (
      nodeData.name == 'IterativeLatentUpscale' ||
      nodeData.name == 'IterativeImageUpscale' ||
      nodeData.name == 'RegionalSampler' ||
      nodeData.name == 'RegionalSamplerAdvanced'
    ) {
    ...
    if (nodeData.name == 'ImpactControlBridge') {
      ...
      nodeType.prototype.onConnectionsChange = function (
        ...
        if (index != 0 || !link_info || this.inputs[0].type != '*') return
    ...
    if (
      nodeData.name == 'ImpactConditionalBranch' ||
      nodeData.name == 'ImpactConditionalBranchSelMode'
    ) {
      const onConnectionsChange = nodeType.prototype.onConnectionsChange
      nodeType.prototype.onConnectionsChange = function (
        type,
        ...
        if (!link_info || this.inputs[0].type != '*') return
    ...
    if (nodeData.name == 'ImpactCompare') {
      ...
      nodeType.prototype.onConnectionsChange = function (
        ...
        if (!link_info || this.inputs[0].type != '*' || type == 2) return
    ...
    if (
      nodeData.name === 'ImpactMakeImageList' ||
      nodeData.name === 'ImpactMakeImageBatch' ||
      nodeData.name === 'ImpactMakeMaskList' ||
      nodeData.name === 'ImpactMakeMaskBatch' ||
      nodeData.name === 'ImpactMakeAnyList' ||
      nodeData.name === 'CombineRegionalPrompts' ||
      nodeData.name === 'ImpactCombineConditionings' ||
      nodeData.name === 'ImpactConcatConditionings' ||
      nodeData.name === 'ImpactSEGSConcat' ||
      nodeData.name === 'ImpactSwitch' ||
      nodeData.name === 'LatentSwitch' ||
      nodeData.name == 'SEGSSwitch'
    ) {
      ...
      nodeType.prototype.onConnectionsChange = function (
        ...
        if (type == 2) {
          ...
          if (connected && index == 0) {
            if (
              nodeData.name == 'ImpactSwitch' &&
              app.graph._nodes_by_id[link_info.target_id]?.type == 'Reroute'
            ) {
              app.graph._nodes_by_id[link_info.target_id].disconnectInput(
                link_info.target_slot
              )
          ...
          if (
            nodeData.name == 'ImpactSwitch' &&
            app.graph._nodes_by_id[link_info.origin_id].type == 'Reroute'
          )
            this.disconnectInput(link_info.target_slot)
  ...
  nodeCreated(node, app) {
    ...
    if (
      node.comfyClass == 'ImpactSEGSLabelFilter' ||
      node.comfyClass == 'SEGSLabelFilterDetailerHookProvider'
    ) {
      node.widgets[0].callback = (value, canvas, node, pos, e) => {
        if (
          node.widgets[1].value.trim() != '' &&
          !node.widgets[1].value.trim().endsWith(',')
        )
          node.widgets[1].value += ', '

        node.widgets[1].value += value
        if (node.widgets_values) node.widgets_values[1] = node.widgets[1].value
      }
    ...
    if (node.comfyClass == 'UltralyticsDetectorProvider') {
      ...
      node.onDrawForeground = function (ctx) {
        ...
        let is_seg =
          model_name_widget.value?.startsWith('segm/') ||
          model_name_widget.value?.includes('-seg')
        if (!is_seg) {
    ...
    if (
      node.comfyClass == 'ImpactWildcardEncode' ||
      node.comfyClass == 'ImpactWildcardProcessor' ||
      node.comfyClass == 'ToDetailerPipe' ||
      node.comfyClass == 'ToDetailerPipeSDXL' ||
      node.comfyClass == 'EditDetailerPipe' ||
      node.comfyClass == 'EditDetailerPipeSDXL' ||
      node.comfyClass == 'BasicPipeToDetailerPipe' ||
      node.comfyClass == 'BasicPipeToDetailerPipeSDXL'
    ) {
